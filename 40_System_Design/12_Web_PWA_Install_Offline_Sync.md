...
`plans/S1/S1.B/pwa-install-offline-sync-research/`
...
Beyond transactions, the web client merges **FIFO outbox** rows into local reads for **categories, tags, sources** (`lookupsOutboxOverlay.ts` → `api/lookups.ts`), **upcoming expenses** (`upcomingOutboxOverlay.ts` → `api/upcomingExpenses.ts` + unpaid names via `listUpcomingExpenses`), **app profile** (`profileOutboxOverlay.ts` → `api/profile.ts`), and extends **snapshot `source_balances`** with queued source creates/renames/deletes. **Online** allowlisted mutators also receive **`Idempotency-Key`** from the Axios request interceptor (`api/client.ts`) so retries do not duplicate rows. See parent sprint notes `plans/S1/S1.B/pwa-implementation-branch/` and research `plans/S1/S1.B/pwa-install-offline-sync-research/D2_API_OUTBOX_CONTRACT.md`.
...
- S1.B sprint index: `plans/S1/S1.B/README.md` (parent repo), anchor **PWA sprint activation**.
