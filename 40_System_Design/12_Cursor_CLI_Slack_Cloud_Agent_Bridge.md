...
PLAN_ROOT: <after planning: plans/cursor/<phase-stage>/<sub-plan>/ — must match governed plan layout; see governance/README.md>
TASK: |
...
1. **roadmap-rollout-planning**
  - Produce an **execution-ready** plan under `plans/cursor/<phase-stage>/<sub-plan>/` (one directory per batch; git branch follows `governance/branching_guidelines.md`).  
...
• Workflow: *roadmap-rollout-planning* → plan under `plans/cursor/<phase-stage>/<sub-plan>/` (see `governance/`) → *orchestration-manager* → implement on a *feature branch* → PR → *design-docs-sync*.
