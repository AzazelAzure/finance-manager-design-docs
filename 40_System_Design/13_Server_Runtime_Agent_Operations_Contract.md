...
- Plan/task packet exists under `plans/cursor/<phase-stage>/<sub-plan>/` (active) or a documented path under `plans/archived/` (read-only evidence), and is listed or cross-linked from `governance/plan_registry.md` when the work is governed.
