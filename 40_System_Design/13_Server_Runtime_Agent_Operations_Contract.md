...
2. **Script-first execution**
   - Use project scripts (`scripts/fm_docker.sh`, `scripts/fm_server_beta.sh`, `scripts/server/*`).
...
- `scripts/fm_docker.sh {status,start,stop,restart,rebuild}`
...
- Plan/task packet exists under `plans/cursor/<phase-stage>/<sub-plan>/` (active) or a documented path under `plans/archived/` (read-only evidence), and is listed or cross-linked from `plans/_governance/plan_registry.md` when the work is governed.
