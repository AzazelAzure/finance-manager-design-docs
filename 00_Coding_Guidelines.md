...
All tasks must adhere to these 15 Golden Rules:
...
- **Primary Source of Truth**: The live files in `design_docs/`, subrepo `CHANGELOG.md`, **`plans/cursor/strategic-roadmap-reframe-53be/`** (strategy), **`governance/`** (plan ops + vocabulary), and other material under `plans/` as referenced by the registry.
